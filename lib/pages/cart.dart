...
class _CartPageState extends State<CartPage> {
  ...
  Future<void> _handlePlaceOrder() async {
    ...
    try {
      ...
      // 4. Place Order via DatabaseMethods (Using the Split-Order Logic)
      final result = await DatabaseMethods().placeOrder(
        userId: widget.userId,
        cartItems: List<Map<String, dynamic>>.from(cartProvider.cart),
        cartProvider: cartProvider, // Pass provider to clear it on success
                ...
                return Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        'Your cart is empty!',
                        style: textTheme.headlineSmall?.copyWith(color: Colors.blueGrey, fontWeight: FontWeight.bold),
                      ),
                    ],
                  ),
                );
  ...
  Widget _buildCartItem(BuildContext context, Map<String, dynamic> item, CartProvider provider) {
    final theme = Theme.of(context);
    final textTheme = theme.textTheme;
    final colorScheme = theme.colorScheme;
          ...
          Expanded(
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  Text(item['Name'] ?? 'No Name', style: textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold), maxLines: 1, overflow: TextOverflow.ellipsis),
                  const SizedBox(height: 6),
                  Text('₹${item['Price']}', style: textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold, color: colorScheme.primary)),
                  if (inventory < 10)
                    Padding(
                      padding: const EdgeInsets.only(top: 4.0),
                      child: Text(inventory > 0 ? 'Only $inventory left!' : 'Out of Stock', style: textTheme.bodySmall?.copyWith(color: inventory > 0 ? Colors.orange : colorScheme.error, fontWeight: FontWeight.bold)),
                    ),
                    ...
                    children: [
                      _buildQuantityButton(
                        ...
                        onPressed: () {
                          ...
                          provider.updateQuantity(widget.userId, productId, quantity - 1, inventory);
                        },
                      ),
                      Padding(
                        ...
                        child: isLoading 
                          ...
                          : Text('$quantity', style: textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
                      ),
                      _buildQuantityButton(
                        icon: Icons.add,
